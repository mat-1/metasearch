'use strict';
var Lexer = require('./lexer'), Expression = require('./expressions').Expression, Fraction = require('./fractions'), Equation = require('./equations');
var Parser = function () {
...
Parser.prototype.parseTermRest = function (factor) {
    if (this.match('multiply')) {
        ...
        powfactor = parseInt(powfactor.toString());
        if (powfactor > 50) {
            throw new Error('nope');
        }
        //WORKAROUND: algebra.js only allows integers and fractions for raising
        return this.parseTermRest(factor.pow(powfactor));
    }
